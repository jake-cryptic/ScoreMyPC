...
if (!empty($_POST)) {
	...
	if (!empty($_POST["RAM"]) && !empty($_POST["CPU"]) && !empty($_POST["GPU"]) && !empty($_POST["BSE"]) && !empty($_POST["NIC"]) && !empty($_POST["HDD"]) && !empty($_POST["USR"]) && !empty($_POST["INF"])) {
		...
		$sUSR = $conn->real_escape_string($usr);
		$sINF = $conn->real_escape_string($inf);
		$time = time();
		
		// Create and execute query
		$query = "INSERT INTO results (`timestamp`,`e_info`,`e_cpu`,`e_ram`,`e_gpu`,`e_bse`,`e_nic`,`e_hdd`,`e_usr`) 
		VALUES ('{$time}','{$sINF}','{$sCPU}','{$sRAM}','{$sGPU}','{$sBSE}','{$sNIC}','{$sHDD}','{$sUSR}')";
